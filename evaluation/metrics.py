...
class StdoutTee:
    """Duplicate stdout to multiple streams (e.g., console and a log file)."""
...
def calculate_metrics(predictions, labels, config):
    ...
    # Setup tee logging to the same directory as Bland-Altman plots
    ...
    try:
        if config.TOOLBOX_MODE == 'train_and_test' or config.TOOLBOX_MODE == 'only_test':
            ...
            exp_name = ''
        save_dir = os.path.join(config.LOG.PATH, exp_name, 'bland_altman_plots')
        os.makedirs(save_dir, exist_ok=True)
        log_path = os.path.join(save_dir, 'metrics_log.txt')
        log_file = open(log_path, 'a', buffering=1)
        sys.stdout = StdoutTee(orig_stdout, log_file)
        print(f"===== Metrics run start {datetime.now().isoformat()} =====")
    except Exception:
        log_file = None
    try:
        ...
        if config.INFERENCE.EVALUATION_METHOD == "FFT" or config.INFERENCE.EVALUATION_METHOD == "MY_FFT":
